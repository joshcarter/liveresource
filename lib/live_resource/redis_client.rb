...
module LiveResource
  def self.redis=(redis)
    ...
    if @proto_redis_client.nil?
      ...
      @proto_redis_client = RedisClient.new(logger, redis)
    else
  ...
  def self.redis
    ...
    if @redis_clients[Thread.current].nil?
      if @proto_redis_client.nil?
        ...
        @proto_redis_client = RedisClient.new(logger)
      end
...
module LiveResource
  ...
  #   def attribute_set(key, value, options = {})
  #     key = "#{@namespace}.#{key}"
  #     value = YAML::dump(value)
  # 
  #     # Don't publish duplicate states as long as there's no TTL
  #     return if (value == @redis[key]) and options[:ttl].nil?
  # 
  #     debug "set", key, value
  #     @redis[key] = value
  # 
  ...
  # 
  #     @redis.expire(key, ttl) if options[:ttl]
  # 
  #     value
  #   end
  #   
